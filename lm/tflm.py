...
"""Example / benchmark for building an LSTM model.

Trains the model described in:
(Zaremba, et. al.) Recurrent Neural Network Regularization
http://arxiv.org/abs/1409.2329

There are 4 supported model configurations: small, medium, large, test
...
flags.DEFINE_string("custom_embeddings", None, "A file containing embeddings."
                    "Size must match network params.")
FLAGS = flags.FLAGS
BASIC = "basic"
CUDNN = "cudnn"
BLOCK = "block"
...
class LMInput(object):
  ...
  def __init__(self, config, data, name=None):
    ...
    self.input_data, self.targets = reader.lm_producer(
...
class LMModel(object):
  ...
  def __init__(self, is_training, config, input_, is_inference):
    ...
    #This is for interence, where input is not provided ahead of time.
    if input_:
    ...
    with tf.device("/cpu:0"):
      if FLAGS.custom_embeddings:
        # Load the custom embeddings.
        tmp = []
        with open(FLAGS.custom_embeddings) as fin:
          for line in fin:
